...
`r nrow(concordanceCCC)` stable features were selected after concordance analysis with a concordance ≥ 0.95.  
...
more_concordance <- concordance[, 1:2]
more_concordance
```
...
# Asked by Lauren : Survival curve by debulking status
...
rec_data %>%
  # full_join(., clinical) %>%
  select(age_at_diagnosis, tnm_cs_mixed_group_stage, treatment_type, raceeth, debulking_status) %>%
  tbl_summary(by = debulking_status) %>%
  bold_labels() %>% add_p(test.args = tnm_cs_mixed_group_stage ~ list(workspace=2e9)) %>% bold_p(t = 0.05)
```
<br>
...
### 1.As Jaileene: train on full "upfront surgery" and test on "upfront neo"
```{r}
set.seed(1234)
...
### Predict risk group and investigate survival 
Upfront surgery CART
```{r}
...
#### Test 3 : C-index may not be best to look at survival tree
...
roc_adj=survivalROC(Stime=pred_adj$recurrence_time, 
                    ...
                    predict.time =12, # For a 1 year survival
                    method="KM")
roc_adj$AUC
...
AUC for survival at 10, 20, 40, 60, 80 months
```{r}
roc_adj=survivalROC(Stime=pred_adj$recurrence_time, 
                    status=pred_adj$rec_event, 
                    marker = pred_adj$pred_adj,
                    predict.time =c(2, 5, 10, 20, 40, 60, 80), 
                    method="KM")
roc_adj$AUC
roc_neo=survivalROC(Stime=pred_neo$recurrence_time, 
                    status=pred_neo$rec_event, 
...
### Is cox still better when looking at AUC?
```{r}
...
```{r}
set.seed(5678)
adj_split <- initial_split(adj, prop = 1/2)
...
fitsurv <- rpart(surv_model ~ f65flatness + f80com_x_pxl + f85com_z_mm, 
                 data = train_adj, 
                 method = "exp", 
                 control = rpart.control(cp= 0.041, 
                                         xval = 10))
# fitsurv
rpart.plot(fitsurv)
# fancyRpartPlot(fitsurv)
...
neo2 <- neo %>% 
  mutate(risk_group2 = predict(as.party(leaf_rpart), newdata = neo))
test_adj1 <- test_adj %>% 
  mutate(risk_group2 = predict(as.party(leaf_rpart), newdata = test_adj))
```
...
km <- survfit(surv_model ~ risk_group, data = train_adj1)
ggsurvplot(km, data = train_adj1,
           title = "Recurrence Analysis on training set \nupfront surgery",
           font.main = c(20, "bold", "black"),
           font.x = c(18, "bold", "black"),
           font.y = c(18, "bold", "black"),
           font.legend = c(16, "black"),
...
km <- survfit(Surv(test_adj1$recurrence_time, test_adj1$rec_event) ~ risk_group2, data = test_adj1)
ggsurvplot(km, data = test_adj1,
           title = "Recurrence Analysis on testing set \nupfront surgery",
           font.main = c(20, "bold", "black"),
           font.x = c(18, "bold", "black"),
           font.y = c(18, "bold", "black"),
           font.legend = c(16, "black"),
...
a <- concordance.index(x=predict(fitsurv), surv.time = train_adj$recurrence_time, surv.event = train_adj$rec_event,
                            method = "noether", na.rm = TRUE)
# b <- concordance.index(x=predict(fitsurv), surv.time = test_adj$recurrence_time, surv.event = test_adj$rec_event,
#                             method = "noether", na.rm = TRUE)
c <- concordance.index(x=predict(fitsurv, newdata = neo), surv.time = neo$recurrence_time, surv.event = neo$rec_event,
                            method = "noether", na.rm = TRUE)
```
...
AUC for survival at 2, 5, 10, 20, 40, 60, 80 months
```{r}
roc_train_adj=survivalROC(Stime=pred_train_adj$recurrence_time, 
                    status=pred_train_adj$rec_event, 
                    marker = pred_train_adj$pred_train_adj,
...
# II.Compare performance with binary prediction
```{r}
...
# III.With all the variables in the univariable analyses

## Wit the split data
```{r}
more_concordance <- more_concordance %>% 
  mutate(stable_features = str_match(variable, "([a-z][:digit:]*)_*")[,2])

stable_features <- 
  paste0(paste(more_concordance$stable_features, "[a-z]", sep = ""), collapse = "|")
mldata <- rec_data %>% 
  select(treatment_type, recurrence_time, rec_event, matches(stable_features))
...
surv_model <- Surv(train_adj$recurrence_time, train_adj$rec_event)
# surv_model # surv_model is dependent variable
fitsurv <- rpart::rpart(surv_model ~ f139glszm_large + f41minimum_hist + f65flatness + f80com_x_pxl + f81com_y_pxl + f83com_x_mm + f85com_z_mm, 
                        data = train_adj, 
                        method = "exp")
# fitsurv
# plot(fitsurv)
...
# Update formula 
set.seed(1234)
fitsurv <- rpart(surv_model ~ f139glszm_large + f41minimum_hist + f65flatness + f80com_x_pxl + f81com_y_pxl + f83com_x_mm + f85com_z_mm, 
                 data = train_adj, 
                 method = "exp", 
...
### C index / AUC
Same results as previously (same tree)
