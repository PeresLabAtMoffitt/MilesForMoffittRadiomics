...
mldata_recipe <-
  ...
  # update_role(contrastenhancementyn, new_role = "Other") %>%
...
cart_fit <- 
   cart_spec %>%
   fit(has_the_patient_recurred ~ .-mrn-contrastenhancementyn, data = train_data)
...
# logistic regression
```{r}
# model <- glm(recurremce ~ treatment_type + preDx_comorbidities + age_at_Dx + Race + tnm_stage + debulking_status, data = mldata, family = binomial)
