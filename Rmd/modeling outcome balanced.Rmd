...
clinical <- read_rds("/Users/colinccm/Documents/GitHub/Peres/MilesForMoffittRadiomics/clinical.rds") %>% 
  select(mrn, has_the_patient_recurred, w_wo_contrast, rec_event, recurrence_time, os_event, os_time,
         age_at_diagnosis, tnm_cs_mixed_group_stage, treatment_type, raceeth)
...
The patient ....... has 2 “biggest” tumor  
1 patient has NA in contrast/no contrast variable  
...
From 538 radiomics tumor info,  
 - 386 are biggest tumor (-1 bc of 1 patient as 2 “biggest” tumor)
 - 345 patients are common with clinical data (aka -19 chemo only, -20 surg only, -3 well diff ; 1 is surg+well diff)  

**We have now 345 patients in our cohort.**
...
75 stable features were selected after concordance analysis with a concordance ≥ 0.95.

## Heatmap
```{r heatplot, fig.width=12}
library(gplots)

df1 <- as.data.frame(mldata %>% full_join(., clinical %>% 
                                            select(mrn, tnm_cs_mixed_group_stage, raceeth)) %>% 
                       select(-w_wo_contrast)) %>% 
...
```{r}
rec_data %>% select(matches("^f[0-9]"), has_the_patient_recurred_) %>% 
...
A strata is applied to the split function to balance w_wo_contrast in both sets.  
```{r}
...
A strata is again applied to the cross validation sets to balance w_wo_contrast.  
...
rf_results %>% # Compare both models
  collect_metrics() %>% 
  select(".metric", rf_mean = mean, rf_sd = std_err) %>% 
  full_join(., glmnet_results %>% 
              collect_metrics() %>% 
              select(".metric", glmet_mean = mean, glmet_sd = std_err)
            , by = ".metric") %>% 
  full_join(., xgboost_results %>% 
              collect_metrics() %>% 
              select(".metric", xgboost_mean = mean, xgboost_sd = std_err)
            , by = ".metric") %>% 
  full_join(., tree_results %>% 
              collect_metrics() %>% 
              select(".metric", tree_mean = mean, tree_sd = std_err)
            , by = ".metric")
...
rf_results %>% # Compare both models
  ...
  bind_rows(glmnet_results %>% 
              collect_metrics() %>% 
              mutate(model = "glmet")) %>% 
  # select(glmet_mean = mean, glmet_sd = std_err)) %>% 
  bind_rows(xgboost_results %>% 
              collect_metrics() %>% 
              mutate(model = "xgboost")) %>% 
  # select(xgboost_mean = mean, xgboost_sd = std_err)) %>% 
  bind_rows(tree_results %>% 
              collect_metrics() %>% 
              mutate(model = "tree")) %>% 
  # select(tree_mean = mean, tree_sd = std_err)) %>% 
...
rec_data <- mldata %>% 
  left_join(., clinical %>% 
  select(mrn, rec_event, recurrence_time,
         raceeth, age_at_diagnosis, grade_differentiation, treatment_type), 
  by = "mrn")
...
tbl1 <- rec_data %>% select(rec_event, recurrence_time,
  ...
  tbl_uvregression(method = survival::coxph, 
                   ...
                   exponentiate = TRUE) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level") %>%
  bold_labels() %>% italicize_levels()
tbl2 <- coxph(Surv(time = rec_data$recurrence_time, 
             event = rec_data$rec_event) ~ raceeth + age_at_diagnosis + grade_differentiation + treatment_type + f144glszm_zone_, data =  rec_data) %>%
