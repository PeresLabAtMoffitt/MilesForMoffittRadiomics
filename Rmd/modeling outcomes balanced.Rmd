...
mldata <- read_rds("/Users/colinccm/Documents/GitHub/Peres/MilesForMoffittRadiomics/radiomics.rds") %>% 
  ...
  right_join(., clinical %>% 
  select(mrn, has_the_patient_recurred),
             by = "mrn") %>% 
  filter(!is.na(.[2])) %>% 
...
rec_data <- mldata %>% 
  left_join(., clinical %>% 
  select(mrn, rec_event, recurrence_time, treatment_type,
         debulking_status), 
  by = "mrn")
```
...
**We have now 296 patients in our cohort.**  
<br>
...
cart_fit <- 
   cart_spec %>%
   fit(has_the_patient_recurred ~ .-mrn, data = train_data)
...
final_xgboost <- xgboost_workflow %>% 
  finalize_workflow(select_best(xgboost_tune, "roc_auc")) #roc_auc
final_xgboost
```
<br>
...
final_rf <- ranger_workflow %>% 
  finalize_workflow(select_best(ranger_tune, "accuracy"))# acc
final_rf
```
<br>
...
final_glmnet <- glmnet_workflow %>% 
  finalize_workflow(select_best(glmnet_tune, "roc_auc")) #roc_auc
final_glmnet
```
<br>
...
final_kknn <- kknn_workflow %>% 
  finalize_workflow(select_best(kknn_tune, "accuracy"))# acc
final_kknn
```
